...
def train():
    ...
    def preprocess_dataset(examples):
        """确保所有文本都在最大长度之内"""
        for key in examples:
            if isinstance(examples[key], str) and len(tokenizer.encode(examples[key])) > MODEL_MAX_LENGTH:
                print(f"警告: 发现长度超过{MODEL_MAX_LENGTH}的序列，将被截断")
        return examples
    
    if isinstance(train_dataset, datasets.Dataset):
        train_dataset = train_dataset.map(preprocess_dataset, batched=False)
